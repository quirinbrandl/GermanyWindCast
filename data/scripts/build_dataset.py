...
def get_meta_zip_links(url, station_ids):
    """
    Scrape the given URL for metadata zip file links matching the station IDs.

    Parameters:
        url (str): The URL of the DWD metadata directory.
        station_ids (list of str): The list of station IDs to include.

    Returns:
        list: A list of full URLs to zip files that match.
    """
    links = [f"Meta_Daten_zehn_min_ff_'{station_id}'.zip" for station_id in station_ids]
...
def build_metadata_dataset(meta_download_dir, station_ids, output_path, max_end_date):
    ...
    if meta_df_list:
        ...
        most_recent_meta = combined_df.sort_values("valid_from").groupby("id").tail(1)

        output_dir = os.path.dirname(output_path)
        os.makedirs(output_dir, exist_ok=True)
        most_recent_meta.to_csv(output_path, index=False)
...
def main():
    """
    Main function that orchestrates the entire data download and processing workflow.

    The function:
    1. Sets up URLs and paths
    2. Downloads metadata files
    3. Builds the station metadata dataset
    4. Downloads wind and temperature data files
    5. Builds the complete merged dataset
    """
